...
export const { use: useSync, provider: SyncProvider } = createSimpleContext({
  ...
  init: () => {
    ...
    const inflightStages = new Map<string, Promise<void>>()
    ...
    return {
      ...
      session: {
        ...
        async stages(sessionID: string) {
          const directory = sdk.directory
          const client = sdk.client
          const [store, setStore] = globalSync.child(directory)
          if (store.session_stages[sessionID] !== undefined) return

          const key = keyFor(directory, sessionID)
          const pending = inflightStages.get(key)
          if (pending) return pending

          const promise = retry(() => client.session.stages({ sessionID }))
            .then((stages) => {
              setStore("session_stages", sessionID, reconcile(stages.data ?? [], { key: "partID" }))
            })
            .finally(() => {
              inflightStages.delete(key)
            })

          inflightStages.set(key, promise)
          return promise
        },
        // Fork the session at a stage boundary. The new session restarts from that
        // stage with the earlier stages' history intact; the caller navigates to it.
        async stageJump(sessionID: string, partID: string, restoreFiles?: boolean) {
          const client = sdk.client
          const res = await client.session.stageJump({ sessionID, partID, restoreFiles })
          if (!res.data) throw new Error("stage jump returned no result")
          // The worktree is shared per directory, so a restore changes the files the
          // original session sees too — drop its cached diff rather than show stale rows.
