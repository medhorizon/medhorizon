...
MedHorizon is model-agnostic. A **model** is one inference endpoint the agent routes through; a **provider** is a vendor that exposes models. Routing happens on your machine: with your own keys (the default), each call goes straight from your machine to the provider — no gateway in the middle, no account required.
...
For BYOK, use `OPENAI_API_KEY`, `XAI_API_KEY`, or `META_MODEL_API_KEY`. The managed Meta route sends only your Atlas session token and proxy URL to MedHorizon; Atlas keeps its shared Meta credential server-side.
...
This is not a hardcoded allowlist for your own provider keys. MedHorizon refreshes models.dev at startup and hourly, so newly published models appear without waiting for a MedHorizon release. The OpenAI Codex OAuth provider is the exception: it only shows models verified to work through a ChatGPT subscription, currently including the full GPT-5.6 family.

## Local models

Prefer to run offline? MedHorizon talks to any OpenAI-compatible endpoint on your machine — Ollama, LM Studio, llama.cpp, vLLM, and more:

```bash
medhorizon local add        # detects Ollama / LM Studio, lists + adds their models
medhorizon run --model ollama/llama3.1 "hello"
```
...
```bash
medhorizon models             # grouped by provider, with a routing label per provider
medhorizon models anthropic   # one provider
```
...
```bash
medhorizon run --model anthropic/claude-opus-5 --variant xhigh "Design the ablation study"
medhorizon run --model anthropic/claude-sonnet-5 "Clean up the plotting code"
medhorizon run --model openai/gpt-5.6-terra --variant max "Review the analysis"
medhorizon run --model moonshotai/kimi-k3 "Implement the pipeline"
```
...
```bash
medhorizon login   # defaults to app.syntheticsciences.ai
```
