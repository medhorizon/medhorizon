...
Run `medhorizon agent list` to print the complete set on your install, including utility sub-agents like `explore` and `reviewer`.

## Pick an agent

```bash
# one-shot run with a specific agent
medhorizon run --agent physics "Fit the dispersion relation in data/spectra.csv"
```

In the workspace, switch agents from the session picker. To change the default, set `default_agent` in `medhorizon.json`. Only agents with mode `primary` or `all` can lead a session; `subagent` profiles are reachable only by delegation.

## Create a custom agent

`medhorizon agent create` walks you through it interactively. The non-interactive form:

```bash
medhorizon agent create \
  --path .medhorizon \
...
`--tools "read,grep,glob"` produces a read-only agent that cannot be tricked into editing files. The available tools are `bash`, `read`, `write`, `edit`, `list`, `glob`, `grep`, `webfetch`, `task`, `todowrite`, and `todoread` — run `medhorizon agent create --help` for the list on your version.

## Where agents live

```text
.medhorizon/agent/             # project-scoped
~/.config/medhorizon/agent/    # user-global
<bundled>                       # ships with MedHorizon
```
