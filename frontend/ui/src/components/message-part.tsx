...
// A stage boundary in a long task. Rendered as an inline marker in the transcript
// so the phase structure is visible while reading; the jumpable graph lives in the
// workspace's stage panel.
PART_MAPPING["stage"] = function StagePartDisplay(props) {
  const part = props.part as StagePart
  return (
    <div data-component="stage-part" data-status={part.status}>
      <div data-slot="stage-rule" />
      <div data-slot="stage-body">
        <span data-slot="stage-index">{part.index}</span>
        <span data-slot="stage-name">{part.name}</span>
        <Show when={part.status === "running"}>
          <Spinner />
        </Show>
        <Show when={part.summary}>
          <span data-slot="stage-summary">{part.summary}</span>
        </Show>
      </div>
    </div>
  )
