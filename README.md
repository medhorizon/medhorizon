...
> 每次 agent（或 AI 编码助手）执行会话**之前**，必须先记录当前 git HEAD，作为后续回滚基准。

```bash
git rev-parse HEAD     # 记录回滚基准点，例如：9d37fb7
git log --oneline -1   # 记录当前提交说明
```

**约定：**

- **会话开始前**：`git status` 确认工作区基线，`git rev-parse HEAD` 记录 HEAD（回滚基准）。
- **记录本身不影响会话**：会话正常进行，只有**需要时**才回滚。
- **需要回滚时**：
  - 已跟踪文件的改动 → `git restore <files>` 恢复到记录的 HEAD；
  - 新增的未跟踪文件 → `git clean`（确认无误后再删）或手动删除；
  - 文档/研究产物等不属于本次代码改动的文件，默认保留。
- 回滚基准记录在会话之外（本说明、会话备注或提交信息），不要写进会被修改的文件。
