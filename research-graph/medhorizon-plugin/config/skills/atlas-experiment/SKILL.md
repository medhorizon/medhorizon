...
5. Map stages with MedHorizon `stage` tool (Design gated, Baseline, Execute, Report gated). Do not invent a parallel stage store. Plugin auto-lands Research Graph nodes; use `atlas_stage` `land` only for recovery.
