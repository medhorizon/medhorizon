...
> **Status: ⛔ Superseded — 2026-08-01.** Stage and Research Graph now replace the Atlas product experience. Follow [`tasks/plans/15-atlas-surface-retirement.md`](../../tasks/plans/15-atlas-surface-retirement.md) to hide the Atlas UI, disable cloud behavior by default, and retain only capabilities proven to be used by Stage/Research Graph. The analysis below is a historical snapshot; do not implement A1–A6.

Workstream: make the primary Atlas workflows — connect, sync, wallet, managed compute — noticeably smoother. This is the **connective-tissue** workstream: it folds in the concrete fixes from [02 codex](<not to do/02-codex-oauth.md>), [03 sync](<not to do/03-atlas-sync.md>), [06 compute](06-compute-integrations.md), and [08 wallet](08-wallet-usage-settings.md), and adds the cross-cutting glue those don't own. Citations `file:line`.
...
- **Sync** — the per-command staleness probe + `syncServices()`, with real correctness bugs (shell-export clobber → billing flip; 402 strands stale creds; non-atomic writes) — see [03](<not to do/03-atlas-sync.md>).
...
- **Codex** — tokens are pushed to Atlas but have **zero consumers** (status-dot only) — see [02](<not to do/02-codex-oauth.md>).
...
4. **Entitlement changes are invisible** — a lapsed plan silently strands managed creds until a call 402s (see [03](<not to do/03-atlas-sync.md>)); nothing proactively tells the user "your Atlas plan changed."
...
- **A4 — Surface entitlement + spend changes** — land [03](<not to do/03-atlas-sync.md>) P4 (visible 402/entitlement notice) and [08](08-wallet-usage-settings.md) (wallet/usage/auto-topup) so the Atlas state is legible, not discovered via a failed call.
