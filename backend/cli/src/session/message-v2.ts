...
export namespace MessageV2 {
  ...
  // A named boundary in a long-running task, written by the `stage` tool. Unlike
  // step-start (one per model step) this marks a semantic phase of the work, and
  // carries the worktree snapshot taken on entry so the user can resume from here.
  // Deliberately absent from toModelMessages: stage markers are navigation
  // metadata for the user, not context for the model.
  export const StagePart = PartBase.extend({
  ...
  export type StagePart = z.infer<typeof StagePart>
