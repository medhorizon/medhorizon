...
在不重写现有运行时的前提下，把下一阶段工作拆成可独立实现、验证和回退的计划，沿四条主线推进：
...
4. **产品面收敛**：Stage 与 Research Graph 接管研究阶段/图谱体验；Atlas UI 隐藏、云功能默认关闭，只按真实调用图保留兼容能力。
...
| 05  | [UI 反馈通道清理](plans/05-ui-feedback-cleanup.md)                   | P1     | 无                   | 01、02、03、06、07     | Planned     |
...
| A：基础测量与低耦合清理 | 01、02、03、05、06、07、15 | 各计划 focused tests 通过；协议/UI 原语边界确定；Atlas allowlist 冻结  |
...
- 1 个前置护栏与 14 份 active 产品计划的验收标准和 checkpoint 全部完成，并有可复跑证据；已被 15 取代的 Plan 14 不再执行。
