...
- **Coordination:** Plan 07 若先落地则复用其 `AsyncState`；否则本计划保持模块内状态映射，不阻塞 Plan 07 后续统一展示
...
- shell 只负责 module tablist、选中状态、键盘/ARIA 和“访问后保持挂载”；不创建 resource，不解释 item，不提供通用 fetch cache。
...
- 新写入先完成 payload，再提交 metadata；metadata 不完整时条目不能冒充完整记录。
...
- module 自己持有 list/selection/preview 状态；切回 Files 后由 shell keep-mounted，避免丢失选中项和滚动。
- 若 Plan 07 的 `AsyncState` 已可用，仅复用其展示原语；否则保持局部、真实的 loading/empty/error/ready 映射，不创建第二套全局数据层。
...
- 旧 `.dat` 继续可 list/preview/download；新 metadata sidecar 是 additive，回退旧 binary 后 payload 仍可被旧实现读取。
...
| 2 · catalog + API + SDK        | 1–1.5 engineer-days | metadata 兼容、bounded reads、路径边界            |
...
合计约 **3.5–5.5 engineer-days**。不需要大规模重构；主要改动是 2 个小型 backend 边界、1 个 core shell、2 个插入模块和 focused tests。
...
- [ ] SDK 由 `./tooling/repo/generate.ts` 生成，workspace 后续只使用生成 client。
...
**Scope:** M — 3 个 hand-edited implementation files、1 个 test 与生成物
...
**Dependencies:** Tasks 2–3、Checkpoint A
...
- [ ] `(cwd: repository root) bun run typecheck && bun run build`
...
- Task 4 必须等待生成 SDK 与 core shell。
- Task 5 最后收口。Plan 07 若同时修改 `FileExplorer.tsx`，本计划不得碰该文件，因此正常情况下无冲突。
...
| metadata sidecar 与 payload 不一致         | 列表显示幽灵或错误条目                        | payload-first、versioned schema、缺 metadata 的 legacy fallback、真实磁盘测试 |
...
| Plan 07 同时修改 FileExplorer              | 合并冲突或状态回归                            | 本计划 FileExplorer 0 行预算；只在外层 shell 接线                             |
...
- [ ] Session artifacts 以本地 RLMArtifacts 为唯一 v1 事实源，并保留 legacy payload 兼容。
- [ ] typed API 支持分页 list、bounded preview、explicit download，且不泄漏绝对路径。
- [ ] Workspace 只消费生成 SDK；API 变化后已运行 `./tooling/repo/generate.ts`。
