...
- **Status:** 🟡 In progress（Tasks 1–5 已合入 `release/v0.3.20`；Checkpoint B 与 DoD 待续）
- **Priority:** P1
...
- 2026-08-03（p16/task5，Task 5 验证与证据）：完成验收覆盖审计并补齐缺口，除串行 lane 的 workspace E2E 外全部本地验证。**后端 focused**：`(cwd: backend/cli) bun test test/session/rlm-artifacts.test.ts test/server/session-artifacts.test.ts` → **14 pass / 0 fail**（13 个既有 + 新增 1 个 `OPENSCIENCE_ENABLE_ATLAS` on/off 等价契约测试：同一真实 route 在 flag on/off 下返回逐字段相同 list/preview/content，且零外部/Atlas/upstream `fetch`）。**workspace typecheck**：`(cwd: frontend/workspace) bun run typecheck`（`tsgo -b`）通过；`(cwd: backend/cli) bun run typecheck` 通过；`e2e/tsconfig.json` 独立 typecheck 通过。**新增覆盖（均在计划已列文件内）**：(1) `e2e/session-artifacts.spec.ts` 新增 3 测试——空 session 成功空态（非错误）+ 点击 refresh 呈现新注册 artifact 且 `page.on("load")` 计数不变（无整页 reload）；40 KiB+ payload 列表正常、preview 标记 truncated 且内容为 32 KiB 有界切片；session A→B 侧栏 in-app 切换后 B 仅见自身 artifact、A 的 row/selection/preview 全部消失、返回 A 按新 scope 重载。(2) 新增 `frontend/workspace/src/features/explorer/ExplorerShell.test.tsx` 单元测试（bun + happy-dom，沿用 Plan 07 frontend/ui render 桥）锁定 module array 移除后的 Files-only 退化边界：单模块注入无 tablist/tab/tabpanel、无陈旧 artifacts 面板，仅 Files 表面；shell 只渲染宿主注入的模块（3 pass）。**待串行 lane**：未运行 workspace E2E suite（共享 `.env.local`）；`e2e/file-explorer.spec.ts`（Files 导航/搜索/打开无回归 + 零 Atlas）、`e2e/session-artifacts.spec.ts`（含上述 3 个新增）、`e2e/atlas-retirement.spec.ts`（Atlas account/bridge/billing 零请求）均待 `(cwd: frontend/workspace) bun run test:e2e` 由后续串行 lane 执行；backend/CLI full `bun test` 与仓库根 `bun run typecheck`/`bun run build` 亦列入 DoD 待验证项。
- 2026-08-03（串行 E2E lane 结果）：`e2e/session-artifacts.spec.ts` **4/4 通过**（list/preview/download、空态+refresh、超限 preview、session A→B 切换）；`e2e/file-explorer.spec.ts` **1/1 通过**（Files 无回归 + 零 Atlas）。Task 4 验证暴露并修复一个真实 bug：`ArtifactPreview` 的 truncated badge 在 `preview.error` 时仍调用 `preview()`（Solid `createResource` accessor 重抛错误），session A→B 切换时旧 artifact 的 404 未捕获 → 应用级错误边界崩溃（整页 "something went wrong"）；已加 `!preview.error` 守卫。`e2e/atlas-retirement.spec.ts` test 1（右栏默认 Research Graph）在本机 E2E 因 Research Graph sidecar 不可用（`research-graph` tab 未渲染，右栏落到 stages）而失败——环境相关（P03 sidecar/网络），非本计划回归；该 spec 的零 Atlas 断言由 session-artifacts 与其它 spec 覆盖。`tasks/todo.md` 已同步勾选 Task 5 项。
