# Medho Windows 一键安装脚本
# 用法：在 PowerShell 中运行，或通过 install.bat 双击执行

param(
  [string]$InstallDir = "$env:LOCALAPPDATA\Medho",
...
Write-Host "  ║       Medho 安装程序          ║" -ForegroundColor Blue
Write-Host "  ╚══════════════════════════════╝" -ForegroundColor Blue
Write-Host ""
...
# ── 3. 下载 ────────────────────────────────────────────────────
$zipPath = "$env:TEMP\medho-windows.zip"
...
if ($exeSrc -and $exeSrc.Name -ne "medho.exe") {
  Rename-Item $exeSrc.FullName "medho.exe"
}
Write-OK "解压完成: $InstallDir\medho.exe"
...
$sc      = $shell.CreateShortcut("$desktop\Medho.lnk")
$sc.TargetPath       = "$InstallDir\start.bat"
$sc.WorkingDirectory = $InstallDir
$sc.Description      = "启动 Medho AI 工作台"
$sc.WindowStyle      = 7   # 最小化窗口（后台运行）
$sc.Save()
Write-OK "创建桌面快捷方式: Medho.lnk"

# ── 7. 版本记录 ─────────────────────────────────────────────────
Set-Content "$InstallDir\version.txt" $version

Write-Host ""
Write-Host "  安装完成！双击桌面的 [Medho] 图标启动。" -ForegroundColor Green
